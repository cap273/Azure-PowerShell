--Create DatabaseBackup - USER_DATABASES - FULL Job
...
SELECT @URLPath = 'https://' + @storname + '.blob.core.windows.net/' + @servername + '-userdbbkp'
...
SET @StepCommand = N'sqlcmd -E -S $(ESCAPE_SQUOTE(SRVR)) -d DBA_Local -Q "EXECUTE [dbo].[DatabaseBackup] @Databases = ''USER_DATABASES'', @URL = N''' + @URLPath + ''', @Credential = ''AutoBackup_Credential'', @BackupType = ''FULL'', @Verify = ''Y'', @Compress = ''Y'', @CheckSum = ''Y'', @LogToTable = ''Y''" -b'
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'DatabaseBackup_User_Sch', 
		@enabled=1, 
		@freq_type=4, 
		@freq_interval=1, 
		@freq_subday_type=1, 
