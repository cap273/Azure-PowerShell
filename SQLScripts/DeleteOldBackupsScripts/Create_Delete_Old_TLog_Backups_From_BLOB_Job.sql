--Create_Delete_Old_TLog_Backups_From_BLOB_Job
...
SET @Command = N'C:\Windows\System32\WindowsPowerShell\v1.0\powershell -Command "C:\MicrosoftScripts\DeleteOldBackupsScripts\Delete_Old_Files.ps1" 15 ''bak'' ''' + @servername +  '-userlogbkp'' ''' + @StorageAccountName + ''' ''' + @StorageAccountKey + '''"'

BEGIN TRANSACTION

USE [msdb]

IF EXISTS (SELECT * FROM msdb.dbo.sysjobs WHERE name = N'Delete_Old_TLog_Backups_From_BLOB')
	EXEC msdb.dbo.sp_delete_job @job_name=N'Delete_Old_TLog_Backups_From_BLOB', @delete_unused_schedule=1
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'Delete_Old_TLog_Backups_From_BLOB', 
		@enabled=1, 
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'Delete_Old_TLog_Backups_From_BLOB_Sch', 
		@enabled=1, 
		@freq_type=4, 
		@freq_interval=1, 
		@freq_subday_type=1, 
